...
The problem is caused by a data race where a thread will read the value of `sum` first, then another thread will read it, and only TODO

One can get around this by wrapping the shared variable in an "atomic" type (atomic means)
