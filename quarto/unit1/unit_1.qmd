...
* Using a Julia package with the `using` keyword. In this case, the `Random` package is one of the standard library packages that come with Julia. 
* Creation of a function, using the `function` keyword, and returning a result with `return` keyword.
* A `for` loop.
* A conditional (`if`) statement.
* A few more language artifacts. 
...
function find_max(data)
    max_value = -Inf
    for x ∈ data # Get ∈ by typing \in + [TAB]. Could have just used `in` as well
        if x > max_value
            max_value = x
        end
    end
...
Note: You rarely need to implement such elementary functions, since most are obviously supplied with the language. For example:

```{julia}
maximum(data)
```

You can get help for this function as others by typing `? maximum` in the REPL or Jupyter. 

Related in-built functions that may be of interest are `findmax`, `argmax`, and `max`. Similarly the same functions with `min` in place of max (`minimum`, `findmin`, `argmin`, and `min`). There is also `minmax`, and `extrema`. Look at the help for some of these functions and try some of the examples.  

## Some fun probability

When we scan for the maximum, let's see how many times we get a new maximum.
...
**If the data is i.i.d., how many on average?**
